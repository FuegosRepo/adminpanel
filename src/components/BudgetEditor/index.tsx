...
export function BudgetEditor({ budgetId, onBudgetDeleted }: BudgetEditorProps) {
	...
	const { editedData, updateField, setEditedData } = useBudgetCalculations(
		budget?.budget_data || null,
	);

	const persistedData = persistedBudgetData || budget?.budget_data || null;

	const hasUnsavedChanges = React.useMemo(() => {
		if (!persistedData || !editedData) return false;
		return !isEqual(persistedData, editedData);
	}, [persistedData, editedData]);

	const persistedTotalTTC = persistedData?.totals?.totalTTC;
	const editedTotalTTC = editedData?.totals?.totalTTC;
	const hasRecalculatedTotal =
		typeof persistedTotalTTC === "number" &&
		typeof editedTotalTTC === "number" &&
		Math.abs(persistedTotalTTC - editedTotalTTC) > 0.01;
	...
	if (error) {
		return (
			<div className="text-center p-10 text-lg text-destructive">{error}</div>
		);
	}

	if (!editedData) {
		return (
			<div className="text-center p-10 text-lg text-destructive">
		...
		<div className="w-full max-w-7xl mx-auto p-4 md:p-6 space-y-6">
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-4 border-b-2 border-primary gap-4">
				...
				<div className="flex items-center gap-3">
					<Badge
						variant={getStatusVariant(budget?.status || "draft")}
						className="uppercase"
					>
			...
			<BudgetModals
				...
				addSelectedMaterials={() =>
					addSelectedMaterials(editedData, setEditedData)
				}
				existingItemNames={
					editedData.material?.items.map((i: any) => i.name) || []
				}
			/>

			{hasRecalculatedTotal && (
				<div className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 p-4 text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
					<AlertTriangle className="mt-0.5 h-5 w-5 flex-shrink-0" />
					<div className="space-y-1 text-sm">
						<p className="font-semibold">
							Total recalculado pendiente de sincronización
						</p>
						<p>
							El presupuesto guardado tiene {persistedTotalTTC.toFixed(2)} €,
							pero el editor recalculó {editedTotalTTC.toFixed(2)} €. Guardá los
							cambios para actualizar también el precio visible en Pedidos.
						</p>
					</div>
				</div>
			)}
			...
			{editedData.service ? (
				<ServiceSection
					...
					onDelete={() => actions.removeSection("service")}
				/>
			) : (
			...
			{editedData.material ? (
				...
				<AddSectionPlaceholder
					label="materiales configurados"
					buttonText="+ Agregar Material"
					onAdd={() =>
						actions.addSection("material", {
							items: [],
							tvaPct: 20,
							totalHT: 0,
							tva: 0,
			...
			{editedData.boissonsSoft ? (
				<BoissonsSoftSection
					...
					onUpdate={updateField}
					onDelete={() => actions.removeSection("boissonsSoft")}
				/>
			) : (
				<AddSectionPlaceholder
					label="bebidas soft configuradas"
					buttonText="+ Agregar Boissons Soft"
					onAdd={() =>
						actions.addSection("boissonsSoft", {
							pricePerPerson: 0,
							totalPersons: 0,
							totalHT: 0,
							tva: 0,
			...
			{editedData.deplacement ? (
				<DeplacementSection
					...
					onUpdate={updateField}
					onDelete={() => actions.removeSection("deplacement")}
				/>
			) : (
				<AddSectionPlaceholder
					label="desplazamiento configurado"
					buttonText="+ Agregar Desplazamiento"
					onAdd={() =>
						actions.addSection("deplacement", {
							distance: 0,
							pricePerKm: 0,
							totalHT: 0,
							tva: 0,
			...
			{editedData.extras ? (
				...
				<AddSectionPlaceholder
					label="extras configurados"
					buttonText="+ Agregar Extra"
					onAdd={() =>
						actions.addSection("extras", {
							items: [],
							totalHT: 0,
							totalTVA: 0,
							totalTTC: 0,
			...
			{!orderId && !loading && (
				<MigrationBanner
					saving={saving}
					createLinkedOrder={createLinkedOrder}
					editedData={editedData}
				/>
			)}
			...
			<BudgetActions
				...
				hasRecalculatedTotal={hasRecalculatedTotal}
			/>
		</div>
	);
}
