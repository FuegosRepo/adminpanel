...
import type { BudgetModalsState } from "./useBudgetModals";
...
interface UseBudgetActionsParams {
	...
	deleteBudgetApi: () => Promise<{ success: boolean; error?: unknown }>;
	...
	openModal: (name: keyof BudgetModalsState) => void;
...
export function useBudgetActions({
	...
	const handleApproveAndSend = async () => {
		if (hasUnsavedChanges) {
			toast.error(
				"⚠️ Tienes cambios sin guardar. Por favor guarda el presupuesto antes de aprobar.",
			);
			return;
		}
		if (!budget?.pdf_url) {
	...
	const confirmApproveAndSend = async () => {
		const result = await approveAndSend(
			editedData.clientInfo.email,
			editedData.clientInfo.name,
		);
		if (result.success) {
	...
	const handleGeneratePDF = async () => {
		if (hasUnsavedChanges) {
			toast.error(
				"⚠️ Tienes cambios sin guardar. Por favor guarda antes de generar el PDF.",
			);
			return;
		}
		...
		if (result.success) {
			if (result.pdfBlob) {
				...
				const filename =
					result.pdfFilename ||
					`Devis_${editedData.clientInfo.name.replace(/\s+/g, "_")}.pdf`;
	...
	const handleMarkAsSent = async () => {
		if (hasUnsavedChanges) {
			toast.error(
				"⚠️ Tienes cambios sin guardar. Por favor guarda el presupuesto antes de marcar como enviado.",
			);
			return;
		}
		if (!budget?.pdf_url) {
