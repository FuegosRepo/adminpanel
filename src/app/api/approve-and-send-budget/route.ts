...
export async function POST(request: NextRequest) {
	try {
		...
		console.log(
			`📧 Aprobando y enviando presupuesto ${budgetId} a ${clientEmail}`,
		);
		...
		if (!budget.pdf_url) {
			return NextResponse.json(
				{
					error:
						"El presupuesto no tiene PDF generado. Por favor, genere el PDF primero.",
				},
				{ status: 400 },
			);
		}
		...
		// ✅ ALSO sync catering_orders status and displayed price if order_id exists
		if (budget.order_id) {
			...
			if (orderUpdateError) {
				console.warn("⚠️ Failed to sync order status/price:", orderUpdateError);
			} else {
				console.log("✅ Order status and price synced to ENVIADO");
			}
		}
		...
		try {
			...
			const pdfPath = budget.pdf_url
				.split("/storage/v1/object/public/budgets/")[1]
				?.split("?")[0];
		...
		if (!config.valid) {
			console.warn(
				`⚠️ ${config.error}. El presupuesto se aprobará pero no se enviará email.`,
			);
		...
		try {
			...
			const logoUrl =
				process.env.EMAIL_LOGO_IMAGE_URL ||
			...
			if (pdfBuffer) {
				const filename =
					budget.pdf_url.split("/").pop()?.split("?")[0] || "devis.pdf";
				attachments.push({
					filename: filename,
				...
				console.log(
					`📎 Adjuntando PDF: ${filename} (${pdfBuffer.length} bytes)`,
				);
			}
			...
			console.log(
				"✅ Email con PDF adjunto enviado al cliente:",
				result.messageId,
			);
			...
			return NextResponse.json(
				{
					success: false,
					error:
						"Error al enviar el email. El presupuesto no se marcó como enviado.",
