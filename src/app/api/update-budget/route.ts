...
export async function POST(request: NextRequest) {
	try {
		...
		const { budgetId, budgetData, editedBy, changesSummary } =
			await request.json();
		...
		if (updatedBudget.order_id) {
			console.log(
				`🔄 Sincronizando cambios al order ${updatedBudget.order_id}...`,
			);
			...
			if (orderUpdateError) {
				budgetLogger.error(
					"Budget saved but parent order sync failed",
					undefined,
					{
						action: "sync_order_failed",
						budgetId,
						orderId: updatedBudget.order_id,
				...
				return NextResponse.json(
					{
						error:
							"Presupuesto actualizado, pero no se pudo sincronizar el precio del pedido",
